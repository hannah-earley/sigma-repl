...
{-
collate :: (Ord r, Aliasable e) => Group e (CRef r) -> CMap e r
collate g = if sandbox g then sandboxed else draft
  where
    entries = foldl promote (Map.fromList $ defs g) $ promotes g
    childs = foldr (\c l -> collate c : l) [entries] $ children g

    scope = foldl combine Map.empty $ zip [1,3..] childs
    combine l (n, r) = Map.union r' l'
      where
        r' = prefixes (CGroup n) r
        l' = foldl (demote (CGroup (n+1) :)) l $ Map.keys l `intersect` Map.keys r'

    precocious = map (CUpper :) . Set.toList $ exps Set.\\ prms
      where
        exps = Set.fromList . exposed $ Map.keys scope
        prms = Set.fromList . map snd $ promotes g

    draft = foldl (demote (CGroup 0 :)) scope precocious
    sandboxed = foldl (demote (CError :)) draft strips
      where
        strips = map (CUpper :) . Set.toList $ upps Set.\\ exps
        upps = Set.fromList . exposed . concat . map slots $ Map.elems draft
        exps = Set.fromList . exposed $ Map.keys draft
-}
